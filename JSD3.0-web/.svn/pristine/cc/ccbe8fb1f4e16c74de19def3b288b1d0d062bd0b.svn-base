define(function(require,exports,module){
	...
	function ajaxUrl(param){
		$.ajax({
		type:'get',
		url:'/member_guarantee/borrow/storeIndentDetailList.html?page=1&rows=5&'+param+'&randomTime='+(new Date()).getTime(),
		dataType:'json',
				...
				require.async(['/plugins/pager/mypager.css','/plugins/pager/pager'],function(){
					kkpager.generPageHtml({
							...
							click:function(n,total,totalRecords){
					        	$.ajax({
					        		type:"get",
					        		url:"/member_guarantee/borrow/storeIndentDetailList.html?rows=5"+"&randomTime="+ (new Date()).getTime()+"&page="+n,
