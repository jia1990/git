define(function(require,exports,module){
	var showTableInvest = require('./autoInvestShowTable');
	var search = function(param,tpl){
		...
		$(".c_date_range li").click(function(){
			...
			if( $(".sInput").val() == undefined){
				var param0 = param + "time="+$(this).data("val") + "&status=" + $("#c_autoInvest_status li.c_recharge_this").data("val") ;
			}else{
				var param0 = param + "time="+$(this).data("val") + "&status=" + $("#c_autoInvest_status li.c_recharge_this").data("val") + "&borrowName=" + encodeURIComponent($(".sInput").val()); ;
			}
			showTableInvest.ajaxUrl(param0,tpl);
		});

		//交易状态(结果)
		$("#c_autoInvest_status li").click(function(){
			$(this).addClass("c_recharge_this").siblings().removeClass("c_recharge_this");
		...
		$("#searchBtn").click(function(){
			...
			{
				var param0 = param + "status=" + $("#c_autoInvest_status li.c_recharge_this").data("val");
			}else{
				var param0 = param + "status=" + $("#c_autoInvest_status li.c_recharge_this").data("val") + "&borrowName=" + encodeURIComponent($(".sInput").val());
			}
