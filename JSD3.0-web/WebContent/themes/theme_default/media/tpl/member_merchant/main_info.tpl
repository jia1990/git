...
<p class="time_text time_textnight">晚上好, <a hidefocus href="">{{#if realName}}{{realName}}！{{/if}}</a> 夜晚因繁星而美丽，人生因相遇而美好<span>{{#if companyName}}公司名称：{{companyName}}{{/if}}</span></p><ul>
	<li>账户名: <a hidefocus href="/member_borrow/account/asetpre.html" class="strongas" title="{{email}}" >{{userNameCheck email}}</a></li>
	<li>安全等级：<a hidefocus href="/member_borrow/account/asetpre.html" class="gradeas">{{#equal realnameStatus 83}}高{{else}}低{{/equal}}</a></li>
	...
	<li class="con1inlilast" style="position:relative;"><a href="/member_borrow/message/detail.html" style="display:block;width:100%;height:100%;" title="您有{{unReadCount}}条未读消息"></a><span style="color:#FF7800;position:absolute;right:-8px;top:-4px;font-size:12px;">{{unReadCount}}</span></li>
</ul>
