define(function(require, exports, module) {
	...
	//判断是否已经绑定银行卡
	$.ajax({
		url:"/member/cash/bankStatus.html",
		type:"get",
		 async: false,
		dataType:"json",
	...
	$("#drawAmount").blur(function(){
		...
		$.ajax({
	        type:"get",
	        dataType:"json",
	        url:"/member/cash/drawStaticRpCash.html",
	        success:function(json){
	        	var useStaticRp=json.useStaticRp;
	        	if(drawAmount>usableMount+useStaticRp){
	    			$("#cashFee").html("<font style='color:#FC6210'>提现金额不能大于可用余额</font>");
	    		}else if(drawAmount>=100 && drawAmount<=usableMount+useStaticRp){
	    			$.ajax({
	    		        type:"get",
	    		        dataType:"json",
	    		        url:"/member/cash/drawMoneyFee.html?drawAmount="+drawAmount+"&id="+Math.random(),
	    		        success:function(json){
	    		        	if(json.newCashMoney!=undefined&&json.newCashRechargeUsable!=undefined&&json.newCashReturnMoneyUsable!=undefined&&json.newCashAwardUsable!=undefined){
	    			           $("#newCashMoneyAdd").html("提现金额&nbsp;"+json.newCashMoney+" =&nbsp; 充值："+json.newCashRechargeUsable+" + 回款："+json.newCashReturnMoneyUsable+" + 奖励："+json.newCashAwardUsable+"(静态红包："+json.newCashStaticRp+")");
	    			        }
	    			        var drawAmount = json.inAccount;
	    			        var cashFee =json.drawFee;		    	   
	    			        $("#cashFee").html("<font style='color:#FC6210;'>实际到账金额："+drawAmount+"元,提现手续费为:"+cashFee+"元</font>");
