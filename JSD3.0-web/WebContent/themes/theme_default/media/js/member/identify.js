define(function(require,exports,module){
	...
	$(function(){
		...
		/*$(".loginPwd .c_clear").eq(0).hide();
	    $(".loginPwd").append("<div class='y_account_div_a'><a href='javascript:void(0);'>修改</a> </div><div class='c_clear'></div>");
		$(".modifyPhone .c_clear").hide();
		$(".modifyPhone").append("<div class='y_account_div_a'><a href='javascript:void(0);'>修改</a> </div><div class='c_clear'></div>");*/
		if($.trim($("#userEmail").val())==""){
			$(".c_update_detail_upemail").hide();
			//JSDP-206  wcw 2015-07-02 start 
			/*$(".chankEmail .c_clear").eq(0).hide();*/
			...
			$(".updateEmall_div").html("<a  href='/user/activeEmail.html' id='changeEmailButton'>邮箱认证</a><div class='c_clear'></div>");
			$(".updateEmall_div").attr("class","updateEmall_div");
		}else{
			//JSDP-206  wcw 2015-07-02 start 
			/*$(".chankEmail .c_clear").eq(0).hide();*/
			//JSDP-206  wcw 2015-07-02 end
			/*$(".chankEmail").append("<div class='y_account_div_a'><a href='javascript:void(0);' id='changeEmailButton'>修改</a></div><div class='c_clear'></div>");*/
			$(".updateEmall_div").html("<a href='javascript:void(0);' id='changeEmailButton'>修改</a><div class='c_clear'></div>");
		}
	});
	...
	$.ajax({
		...
		success:function(json){
			require.async('/plugins/handlebars-v1.3.0/handlebars-v1.3.0',function(){
				...
				$(".y_account_right_top").html(Handlebars.compile(require("../../tpl/member/accset.tpl"))(json));
			  });
            });
			/*826   fgq  3.18   安全等级*/
	...
	$.ajax({
		...
		success:function(json){
			require.async('/plugins/handlebars-v1.3.0/handlebars-v1.3.0',function(){
	            require.async('/plugins/handlebars-v1.3.0/transFormatJson',function(){      	
	            //  找回 交易密码内容
				$(".c_find_detail").html(Handlebars.compile(require("../../tpl/member/accsetFindPayPwd.tpl"))(json));
				//找回密码方式切换
				...
				$(".ppgetcode").click(function(){
					...
					$("#checkPhoneCodeSubmit").click(function(){
						...
						if (validCode) {
							...
							$.ajax({
								type:"post",
								url:"/member_borrow/account/sendMobileMsg.html",
	...
	$.ajax({
		...
		success:function(json){
			require.async('/plugins/handlebars-v1.3.0/handlebars-v1.3.0',function(){
	            require.async('/plugins/handlebars-v1.3.0/transFormatJson',function(){
				$(".c_update_detail1").html(Handlebars.compile(require("../../tpl/member_borrow/accsetPaypwd.tpl"))(json));
				if(json.result){ // 设置
					$("#paypwdadd_forms").show() ;
					$("#paypwdupdate_forms").hide() ;
					//JSDP-206  wcw 2015-07-02 start 
					/*$(".setpaypwd .c_clear").eq(0).hide();*/
					//JSDP-206  wcw 2015-07-02 end
				...
				if($("#sflag").val()=="setpaypwd"){
					//设置交易密码之前判断是否设置了密保问题
							$.ajax({
								...
								success:function(json){
									if(!json.size>0){
										alertNew("为了您的账户安全，设置交易密码前先设置密保问题","提示",function(){},true);
				...
				if($("#sflag").val()=="updateEmail"){
					$(".c_update_detail_upemail").show();
					$(".c_update_detail_upemail").slideDown(700);
				}
				if($("#sflag").val()=="updatePhone"){
				...
				$(".paypwdSet").click(function(){
						$.ajax({
							...
							success:function(json){
								if(!json.size>0){
									//alert("为了您账户的安全，请先设置密保问题");
									alertNew("为了您账户的安全，设置交易密码前请先设置密保问题","提示",function(){},true);
									$(".paypwdSet").parent().next('.c_update_detail').slideUp(500);
									/*$(".myquestion").click();
									$(".myquestion").parent().parent().find('.c_update_detail').slideDown(500);*/
									$('.y_account_div_a').eq(2).click();
				    			...
				    			require.async('jquery.form',function(){
				    				$(form).ajaxSubmit(function(data){
				    					if(data.result){
				    						...
				    						$(".y_account_div_a").parent().next('.c_update_detail').slideUp(500);
				    						alertNew("交易密码修改成功！","提示",function(){},true);*/
				    						//window.location.reload();
	...
	$.ajax({
		type:"post",
		url:"/user/qqCooperateLoginInfo.html",
		dataType:"json",
		async: false,
		success:function(json){
			if(json.qqCooperateInfo){
				$(".qqCooperateInfo").html("QQ解绑");
				$(".qqCooperateInfo").attr("data-val","/user/QQLoginUnBind.html");
			}else{
				$(".qqCooperateInfo").html("QQ绑定");
				$(".qqCooperateInfo").attr("href","/user/qqLogin.html");
			}
		}
	});
		...
		//qq解绑处理
		$(".qqCooperateInfo").click(function(evt){
			var qqCooperateUrl = $(this).attr("data-val");
			if(qqCooperateUrl=='/user/QQLoginUnBind.html'){
				$.ajax({
					type:"post",
					url:qqCooperateUrl,
					dataType:"json",
	...
	// 获取用户微信绑定信息
		$.ajax({
			type:"post",
			url:"/user/wxCooperateLoginInfo.html",
			dataType:"json",
			async: false,
			success:function(json){
				if(json.wxCooperateInfo){
					$(".wxCooperateInfo").html("微信解绑");
					$(".wxCooperateInfo").attr("data-val","/user/wxLoginUnBind.html");
				}else{
					$(".wxCooperateInfo").html("微信绑定");
					$(".wxCooperateInfo").attr("data-val","/user/bindWx.html");
				}
			}
		});
			...
			//wx解绑处理
		$(".wxCooperateInfo").click(function(){
			var wxCooperateUrl = $(this).attr("data-val");
			//解绑
			if(wxCooperateUrl=='/user/wxLoginUnBind.html'){
				$.ajax({
					type:"post",
					url:wxCooperateUrl,
					dataType:"json",
			...
			}else if(wxCooperateUrl=='/user/bindWx.html'){//绑定操作
				//弹框
				$("#scanWx").show();
					//微信绑定页面弹出
				$(".modal-backdrop").show();
				$.ajax({
					...
					async: false,
					success:function(json){
						var content = eval('(' + json.content + ')');
						var QRCodeUrl = content.url;
						$(".img-polaroid").attr("src",QRCodeUrl);
					}
			...
			//确认按钮点击事件，ajax请求绑定数据，成功则更改状态，不成功两秒刷新一次数据直到成功
		$(".closeBtn").click(function(){
				$("#scanWx").hide();
				$(".modal-backdrop").hide();
				//获取微信绑定信息ture:绑定成功     false:尚未绑定
				var second = 120000;
				var result =wxCooperateInfo();
				if(!result){
					//点击确定时判断是否绑定成功，没有则两秒扫描一次，直到
					var t1 = window.setInterval((function(){
						var result1 = wxCooperateInfo();
						if(result1){
							var t2 = window.clearTimeout(t1); 
							$(".wxCooperateInfo").html("微信解绑");
							$(".wxCooperateInfo").attr("data-val","/user/wxLoginUnBind.html");
						}
						second = second-5000;
	...
	//登录，修改手机，修改邮箱的模板等加载
		...
		success:function(data){
			/*alert(data.user.mobilePhone+"   "+data.user.email);*/
			require.async('/plugins/handlebars-v1.3.0/handlebars-v1.3.0',function(){
	            require.async('/plugins/handlebars-v1.3.0/transFormatJson',function(){
	            	/*$(".updateQuestion").html(Handlebars.compile(require("../../tpl/member_borrow/accsetQuestion.tpl"))(data));*/
	            	$(".c_update_login_pwd").html(Handlebars.compile(require("../../tpl/member/updateLoginPwd.tpl"))(data));
	            	$(".c_update_way").html(Handlebars.compile(require("../../tpl/member/updatePhone.tpl"))(data));
	            	$(".c_update_way2").html(Handlebars.compile(require("../../tpl/member/updateEmail.tpl"))(data));
	            							...
	            							success:function(json){
	            								...
	            								if(!json.result){
	            									...
	            									$("#checkPhoneCodeSubmit").click(function(){
	            										...
	            										$.ajax({
	            											type:"post",
	            											url:"/member_borrow/account/sendMobileMsgUM.html?flag"+newSmsFlag,
	            											dataType:"json",
	            											data:{phone:phone,validCode:$("#sendSmsValidCode").val()},
	            											success:function(json){
	            												//  发送成功
	            												if(json.result){
	            													$(".calculatorCpmBoxPhone").hide();
	            	...
	            	$(".upMsendEmail").click(function(){
						...
						$("#checkPhoneCodeSubmit").click(function(){
							var time=120;
							if (uMvalidEmailCode == true && newvalidCode == true) {
								$.ajax({
									type:"post",
	            	...
	            	$(".newsendEmail").click(function(){
	            		// 获取邮箱/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/;
	            		/*var reg = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/ ;*/
	            		var reg = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,4}$/ ;
	            		var email=$(".c_email_msg").prev().val();
	            		if(email==""){
	            			$(".c_email_msg").html("请输入邮箱地址");
	            		}else{
	            			if(!reg.test(email)){  
	            				$(".c_email_msg").html("邮箱格式错误");
	            			}else if(email==$("#userEmail").val()){
	            				$(".c_email_msg").html("新邮箱与旧邮箱一样，请输入不同的邮箱。");	
	            			}else if(email!=$("#userEmail").val()){
	            				$(".c_email_msg").html("");
	            				$.ajax({
	            					type:"post",
	            	...
	            	/*	var reg = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/ ;*/
	            		...
	            		if(email==""){
	            			$(".c_email_msg1").html("请输入邮箱地址");
	            		}else{
	            			if(!reg.test(email)){  
	            				$(".c_email_msg1").html("邮箱格式错误");
	            			}else if(email==$("#userEmail").val()){
	            				$(".c_email_msg1").html("新邮箱与旧邮箱一样，请输入不同的邮箱。");	
	            			}else if(email!=$("#userEmail").val()){
	            				...
	            				$.ajax({
	            					...
	            					success:function(json){
	            						if(!json.result){
	            							/*$(".c_email_msg1").attr("style","float: right;margin-left: 0;margin-right: 160px") ;
	            							$(".c_email_msg1").html(json.msg) ;*/
	            							$(".c_email_msg1").html(json.msg) ;
	            						}else{
	            							$(".c_email_msg1").html("");
	            							var time=120;
		...
		$(".c_update_type dd").click(function(){
			...
			if(index==1){
				if($.trim($("#userEmail").val())==""){
					...
					alerts("认证信息提示","<div style='display:block;text-align:center;font-size: 18px;'>请先进行邮箱认证</div><br><a href='/user/activeEmail.html' style='margin-right:40px;'>邮箱验证</a><a href='javascript:void(0);' class='y_alertsBoxButton cancelButton' style='background:#ddd;color:#333;'>取消</a>",400,200);
					$(".cancelButton").click(function(){
						$(".c_update_detail_upmobile").find(".c_mobile_valid").eq(0).show();
